...
def stable_hash(payload: Dict[str, Any], *, prefix: str, length: int = 12) -> str:
...
class RnnStateStore:
    ...
    def make_lot_id(self, *, kind: str, seed: Optional[int], requested_count: int, config_json: Dict[str, Any]) -> str:
        return stable_hash(
            {
                "kind": kind,
                "seed": seed,
                ...
                "config": config_json,
                "created_at_hint": utc_now_iso(),
            },
            prefix="lot",
        )

    def create_lot(self, *, lot_id: str, kind: str, config_json: Dict[str, Any], seed: Optional[int], requested_count: int) -> LotRow:
        created_at = utc_now_iso()
        raw_config = json.dumps(config_json, sort_keys=True, ensure_ascii=False)
        with self._connect() as conn:
            conn.execute(
                "INSERT INTO lots (lot_id, kind, config_json, seed, requested_count, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                (lot_id, kind, raw_config, seed, requested_count, created_at),
            )
        return LotRow(lot_id=lot_id, kind=kind, config_json=raw_config, seed=seed, requested_count=requested_count, created_at=created_at)
    ...
    def latest_lot(self) -> Optional[LotRow]:
        with self._connect() as conn:
            row = conn.execute("SELECT * FROM lots ORDER BY created_at DESC LIMIT 1").fetchone()
        if row is None:
            return None
        return LotRow(**dict(row))
    ...
    def add_model_to_lot(self, *, lot_id: str, model_id: str, position: int, source: str) -> None:
        with self._connect() as conn:
            conn.execute(
                "INSERT OR IGNORE INTO lot_models (lot_id, model_id, position, source) VALUES (?, ?, ?, ?)",
    ...
    def upsert_measurement(self, *, model_id: str, runtime: str, profile_id: str, config_json: Dict[str, Any], result_path: str, status: str) -> MeasurementRow:
        measurement_id = stable_hash({"model_id": model_id, "runtime": runtime, "profile_id": profile_id}, prefix="measure")
        created_at = utc_now_iso()
        raw_config = json.dumps(config_json, sort_keys=True, ensure_ascii=False)
        with self._connect() as conn:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(model_id, runtime, profile_id) DO UPDATE SET
        ...
        return MeasurementRow(measurement_id=measurement_id, model_id=model_id, runtime=runtime, profile_id=profile_id, config_json=raw_config, result_path=result_path, status=status, created_at=created_at)
