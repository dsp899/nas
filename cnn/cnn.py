...
class Model():
    ...
    def __init__(self, config, num_classes):
        self.model_filename = f"{config.arch['cnn']}_{config.data['name']}_{config.data['num_train_frames']}_frames_{config.data['size']}_size"
        self.predict_filename = f"{config.arch['cnn']}_{config.data['name']}_{config.data['num_predict_frames']}_frames_{config.data['size']}_size"
        self.models_path = f"{os.path.abspath(os.path.curdir)}/models/keras/{config.data['name']}/cnn/{config.arch['cnn']}/{self.model_filename}"
        self.data_path = f"{os.path.abspath(os.path.curdir)}/data/features/{config.data['name']}/{config.arch['cnn']}/{self.predict_filename}"
    ...
    def load_train(self): 
        if os.path.exists(self.info_path):
            with open(self.info_path, "r") as f:
                ...
                self.current_epoch = 0 if self.current_epoch >= self.config.metadata['epochs'] else self.current_epoch
        else:
        ...
        if self.current_epoch != 0:
            ...
            elif self.config.arch['cnn']  == 'resnet50':
            ...
            elif self.config.arch['cnn']  == 'inceptionV3':
            ...
            dropout = tf.keras.layers.Dropout(self.config.arch['dropout'])(pooling)   
            dense1 = tf.keras.layers.Dense(self.config.arch['dense_units'], activation='relu')(pooling)  
            dropout = tf.keras.layers.Dropout(self.config.arch['dropout'])(dense1) 
    ...
    def train(self, data):
        ...
        save_progress = SaveProgress(self.model,self.config.metadata['batch'],self.config.metadata['lr'],self.current_epoch,self.best_val_loss,self.best_val_acc,self.info_path,self.best_model_path, self.last_model_path)
        early_stop = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5)
        self.model.fit(
            data_train, 
            validation_data=data_test,
            initial_epoch=self.current_epoch, 
            epochs=self.config.metadata['epochs'],
